...
export default ({
    setCreateLBCProductModel,
    createLBCProductModel,
    createLBCProduct,
}) => {
    const [lbcProduct, setLBCProduct] = useState({
        farmer: "",
        location: "",
        quantity: "",
        price: "",
    ...
    const createItem = async () => {
        try {
            await createLBCProduct(lbcProduct);
            console.log(lbcProduct)
        } catch (error) {
            console.log("Wrong creating item");
        }
    };
    return createLBCProductModel ? (
        <div className="fixed inset-0 z-10 overflow-y-auto">
            <div
                className="fixed inset-0 w-full h-full bg-black opacity-40"
                onClick={() => setCreateLBCProductModel(false)}
            ></div>
